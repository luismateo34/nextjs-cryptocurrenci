...
const ItemDashboard = ({ text }: { text: string }) => {
  return (
    <span className="w-10/12 md:w-full text-purple-300 inline-flex bg-black/40 backdrop-blur-md rounded-md px-2 items-center justify-center h-9 md:h-7">
      {text}
    </span>
  );
};
    ...
    >
      ...
      >
        ...
        <div className="w-full h-full flex flex-col  md:pb-2 justify-around items-center gap-2 col-start-1 col-end-2">
          ...
          <button
            className="w-3/5 h-8 rounded-sm my-auto md:m-0 mx-auto text-white bg-pink-800/90 hover:bg-orange-700"
            onClick={() => {
              noShow();
              noselect();
              resetDate();
        ...
        <div className="w-full h-full flex flex-col flex-wrap text-sm  lg:text-base  text-yellow-500  mt-3 md:mt-0 px-2 justify-around items-center gap-2 col-start-1 col-end-2 md:col-start-2 md:col-end-3">
          <ItemDashboard text={symbol} />
          <ItemDashboard text={`price:${price}`} />
          <ItemDashboard text={`contract_type:${contract_type}`} />
          <ItemDashboard text={`market:${market}`} />
          <ItemDashboard text={`volume_24h:${volume_24h}`} />
          <ItemDashboard text={`spread:${spread}`} />
        </div>
      </div>
    </section>
  );
