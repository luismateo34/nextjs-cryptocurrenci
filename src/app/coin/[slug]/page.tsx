...
export default async function Page({
  ...
  const currencyOpt:currency = searchParams?.currency === undefined ? "usd" as currency : searchParams?.currency as currency
  const days = searchParams?.days === undefined ? "30" : searchParams?.days as string
        ...
        data.map((el) => {
          return (
            <div key={el.id}>
              ...
              <span> market_cap_change_24h: { el.market_cap_change_percentage_24h }</span>
              <span> "market_cap_change_percentage_24h: { el.market_cap_change_percentage_24h }</span>
	      <SelectDay/>
	      <SelectCurrency/>
	      <GraficCircule porcentaje={el.price_change_percentage_24h} key={el.id} />
